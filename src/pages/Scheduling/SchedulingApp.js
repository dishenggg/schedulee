    ...
    if (!dragStarted) {
      ...
      const gridsToHide = Object.keys(listOfTripsByDriver).filter(
        (driverId) => {
          ...
          return false; // Display grid if there are no clashes or bus size issues
        }
      );
  ...
  const generateGrids = (drivers, gridStyle) => {
    return [...drivers].map((driver) => {
      ...
      return (
        <div
          key={driverId}
          style={{ ...gridStyle, display: isDisplayed ? "block" : "none" }}
        >
