...
import { addDoc, writeBatch, collection, doc } from 'firebase/firestore';
...
const AddMultipleTrips = ({ drivers, subCons, updateListOfTripsByDriver }) => {
    ...
    const parseRowsToTrips = (parsedRows) => {
        const res = parsedRows.map((row, i) => {
            ...
            if (
                trip.numBusAssigned > trip.numBus ||
                numBusAssigned2 > trip.numBus
            ) {
        ...
        const prepRowForFirebase = (row) => {
            ...
            Object.entries(rowCopy).forEach(([key, value], index) => {
                if (
                    ...
                    rowCopy[key] = parseTimeToDatetime(
                        new Date(rowTripDatetime),
                        value
                    );
                }
            });
